...
**The second State case, and why the reorder helps but does not settle it.** P4 and the elevator
both read State, which contradicts the principle this table exists to enforce — Command was removed
from chess the moment P2 earned it, and Composite was dropped from P2 rather than badge a case with
a pattern it does not use. The duplication is structural rather than accidental: Part 1 section 5.7
names the vending machine *and* the elevator as the prompts that earn State its full page. Raised
with the author before P4 was built; his instruction was to build P4 on whatever patterns fit and
settle the other one when it is commissioned. **P4 takes State outright.**

The author has since **moved chess to P5 and the elevator to P6**, so the two State cases are no
longer adjacent and a case about inheritance versus composition sits between them. That is a
mitigation, not a resolution. When P6 is briefed the open question is unchanged: whether the
elevator's distinct lesson is the **scheduling policy alone**, with the lifecycle presented as
baseline the way P4 presents its own. If it cannot be, the elevator is the wrong tenth of the book
and the slot should go to a problem that is not a state machine.
...
implementable inside a 45-minute round. Prefer concrete types on later problems for the same reason.
